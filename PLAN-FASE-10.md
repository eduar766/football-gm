...
> **Estado: COMPLETADO** — todos los batches implementados y testeados (99/99 tests pasan, 6/6 packages typecheckean limpio).
...
| # | Fix | Archivos | Estado |
|---|-----|---------|--------|
| 1.1 | **Fix bug filter Fase 9**: `processEconomy` filtra por `playerFederationId` (competing, divisions, matchday loop, merchandise) | `economy.ts` | ✅ |
| 1.2 | **Recalibrar ingresos**: matchday 70%→10% (comisión), merchandise 50K→15K/equipo, contratos base ÷3 | `economy.ts` | ✅ |
| 1.3 | **Fix `crisis_economica_club`**: actuaba dando +3M€ al tesoro; ahora cuesta 5M€ (rescate real) | `events.ts` | ✅ |
| 1.4 | **Límite de `callReview`**: máx 2/temporada + −1 prestige por uso (no solo coste económico) | `engine.ts` | ✅ |
| 1.5 | **Fix traspasos**: los fichajes entre clubes no drenan el tesoro federal | `transfers.ts` | ✅ |
| 1.6 | **Upside de normas**: `governanceBonus()` — normas cumplidas dan +1/+2 prestige al cierre | `norms.ts`, `engine.ts` | ✅ |
...
La pieza central del diseño §4.2 que no estaba implementada.

| # | Feature | Archivos | Estado |
|---|---------|---------|--------|
| 3.1 | `NegotiationRequirement` type: `{ tipo: 'prestigio'\|'estadio'\|'reparto', objetivo, cumplido }` | `types.ts`, `contracts` | ✅ |
...
| # | Feature | Archivos | Estado |
|---|---------|---------|--------|
| 5.1 | Motor de titulares: plantillas sobre `matchReports` + `history` (rachas, sorpresas, goleadas) | `headlines.ts` (nuevo), `contracts`, `DashboardPage.tsx` | ✅ |
...
| 6.4 | Separar prestige de fuerza en rivales: `updateRivalPrestige()` con inercia asimétrica (+2/−1 por temporada) | `rival-sim.ts`, `types.ts` | ✅ |
...
| # | Feature | Archivos | Estado |
|---|---------|---------|--------|
| 7.1 | Gráfico de trayectoria de equipos: posición por temporada desde `trajectories` (Recharts LineChart, eje Y invertido) | `HistoryPage.tsx` | ✅ |
| 7.2 | Libro de récords: mayor goleada, racha ganadora más larga — `updateRecordBook()` en `closeSeason` | `engine.ts`, `game.service.ts`, `HistoryPage.tsx` | ✅ |
| 7.3 | Coeficiente histórico de federaciones: `accumulateFederationCoefficients()` + tab "Ranking Mundial" | `engine.ts`, `game.service.ts`, `FederationsPage.tsx` | ✅ |
| 7.4 | Export/import de partida: `GET :id/export` / `POST import` + botones en GamesPage | `game.controller.ts`, `game.service.ts`, `GamesPage.tsx` | ✅ |
...
- `updateRecordBook` se llama **antes** de `s.year += 1` para capturar el año de la temporada recién jugada.
- `accumulateFederationCoefficients` se llama **después** de `computeGlobalRanking` (necesita `s.globalRankings` actualizado).
